...
export const getAllSignalement = async () => {
...
export async function getSignalementRecapitulatif() {
  const res = await fetch(`${API_URL}/api/signalements/recapitulatif`);
...
export async function getSignalementSummary() {
  const res = await fetch(`${API_URL}/api/signalements/summary`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
};

export async function syncFromFirebase() {
  const res = await fetch(`${API_URL}/api/sync/from-firebase`, {
    method: 'POST',
  });
  if (!res.ok) throw new Error(await res.text());
  return res.text();
...
export async function syncToFirebase() {
  const res = await fetch(`${API_URL}/api/sync/to-firebase`, {
    method: 'POST',
  });
  if (!res.ok) throw new Error(await res.text());
  return res.text();
...
export async function fullSync() {
  const res = await fetch(`${API_URL}/api/sync/full`, {
    method: 'POST',
  });
  if (!res.ok) throw new Error(await res.text());
  return res.text();
...
export async function getDelaiTraitementMoyen(statusFinal = 'termine') {
  const res = await fetch(`${API_URL}/api/signalements/delai-traitement-moyen?statusFinal=${encodeURIComponent(statusFinal)}`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
};
